          ...
          <div class="flex-1 flex flex-col gap-1 px-2 py-1.5 overflow-hidden min-h-0">
            ...
            {#if metrics}
              <div class="grid grid-cols-2 gap-x-2 gap-y-0 flex-shrink-0 text-[10px]">
                ...
                <div class="flex justify-between"><span class="text-base-content/40">Speed</span><span class="text-base-content" style="font-variant-numeric: tabular-nums">{(metrics.speed / 32768 * 100).toFixed(1)}%</span></div>
                <div class="flex justify-between"><span class="text-base-content/40">Pos</span><span class="text-base-content" style="font-variant-numeric: tabular-nums">{formatPosition(metrics.currentPosition)}</span></div>
            ...
            <div class="flex gap-1 mt-auto flex-shrink-0">
              {#if timeInfo}
                <span class="badge badge-xs {timeInfo.onAir === 1 ? 'badge-success' : ''}">{timeInfo.onAir === 1 ? "ON AIR" : "OFF AIR"}</span>
              {/if}
              {#if metrics}
                <span class="badge badge-xs {metrics.syncMaster === 1 ? 'badge-warning' : ''}">{metrics.syncMaster === 1 ? "MASTER" : "SLAVE"}</span>
              {/if}
            </div>
          </div>
        {:else}
