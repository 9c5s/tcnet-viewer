...
describe("isValidImageData", () => {
  test("JPEGマジックバイト (0xFF 0xD8) を有効と判定する", () => {
    expect(isValidImageData(Buffer.from([0xff, 0xd8]))).toBe(true);
  });

  test("JPEGマジックバイト + 後続データを有効と判定する", () => {
    expect(isValidImageData(Buffer.from([0xff, 0xd8, 0xff, 0xe0, 0x00]))).toBe(true);
  });
...
describe("processArtworkPacket", () => {
  test("有効なJPEGバッファからbase64とmimeTypeを返す", () => {
  ...
  test("base64を正しくデコードすると元のバッファに戻る", () => {
    const jpeg = Buffer.from([0xff, 0xd8, 0xff, 0xe0, 0xaa, 0xbb, 0xcc]);
    const result = processArtworkPacket(jpeg)!;
